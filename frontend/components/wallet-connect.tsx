...
export default function WalletConnect() {
    ...
    const updateWalletInfo = async (availWallet: any) => {
        if (availWallet) {
            ...
            console.log({ addr })
            const addressesAndPkhs: any = await getUserAddressesAndPkhs(availWallet as string)
            userContext.setUser({ walletName: availWallet as string, address: addr, addressesAndPkhs })
        }
    }
    ...
    useEffect(() => {
        ...
        if (walletName && walletAddress) {
            ...
            getUserAddressesAndPkhs(walletName).then((addressesAndPkhs: any) => {
                userContext.setUser({ walletName, address: walletAddress, addressesAndPkhs })
            })
